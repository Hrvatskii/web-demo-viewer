...
const parseUserCmdMessage = (offset, data) => {
  ...
  let UserCmdData = isolateBytes(data, offset + 13, bitsToInt(data, offset + 9, 4));
  ...
  // pain
  [CommandNumber, index] = ifOneExists(UserCmdData, index, 4, "int", 0, null);
  [TickCount, index] = ifOneExists(UserCmdData, index, 4, "int", 1, null);
  [ViewAnglesX, index] = ifOneExists(UserCmdData, index, 4, "float", 2, null);
  [ViewAnglesY, index] = ifOneExists(UserCmdData, index, 4, "float", 3, null);
  [ViewAnglesZ, index] = ifOneExists(UserCmdData, index, 4, "float", 4, null);
  [ForwardMove, index] = ifOneExists(UserCmdData, index, 4, "float", 5, null);
  [SideMove, index] = ifOneExists(UserCmdData, index, 4, "float", 6, null);
  [UpMove, index] = ifOneExists(UserCmdData, index, 4, "float", 7, null);
  [Buttons, index] = ifOneExists(UserCmdData, index + 1, 4, "int", 0, "button");
